...
public class TransactionsController : ControllerBase
{
    ...
    public TransactionsController(Supabase.Client client)
    {
        _client = client;
    }
    ...
    public async Task<IActionResult> GetAll()
    {
		...
		var userResponse = await _client.From<User>().Where(u => u.Id == userId).Get();
		...
		var response = await _client.From<Transactions>().Get();
    ...
    public async Task<IActionResult> Create([FromBody] Transactions transaction)
    {
        ...
        var response = await _client.From<Transactions>().Insert(transaction);
