...
public class JwtService
{
	...
	public async Task<LoginResponseModel?> Authenticate(LoginRequestModel request )
	{
		...
		var userAccount = await client.From<User>().Where(u => u.Username == request.Username ).Get();
		var user = userAccount.Models.FirstOrDefault();
		if ( user is null /*|| !PasswordHashHandler.VerifyPassword( request.Password, user.Password )*/) return null;
